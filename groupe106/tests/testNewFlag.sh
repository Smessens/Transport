...
dd if=/dev/urandom of=input_file bs=1 count=75000 &> /dev/null

valgrind_sender=""
valgrind_receiver=""
valgrind_link_sim=""
if [ ! -z "$VALGRIND" ] ; then
    valgrind_sender="valgrind --leak-check=full --log-file=valgrind_sender_testNewFlag.log"
    valgrind_receiver="valgrind --leak-check=full --log-file=valgrind_receiver_testNewFlag.log"
    valgrind_link_sim="valgrind --leak-check=full --log-file=valgrind_link_testNewFlag.log"
fi

# On lance le receiver et capture sa sortie standard
$valgrind ./receiver :: 64341 -l -w 25 -s reeiver.csv 2> logFile/receiver_testNEwFlag.log > received_file &
receiver_pid=$!
...
$valgrind ./tests/link_sim -p 64342 -P 64341 -R -d 300 -j 50 2>logFile/link_sim_testNewFlag.log &
link_sim_pid=$!
# On démarre le transfert
if ! $valgrind ./sender ::1 64342 -l -s sender.csv < input_file 2> logFile/sender_testNewFlag.log ; then
  echo "Crash du sender!"
  cat sender_testNewFlag.log
