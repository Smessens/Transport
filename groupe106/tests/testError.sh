...
valgrind_link_sim=""
if [ ! -z "$VALGRIND" ] ; then
    valgrind_sender="valgrind --leak-check=full --log-file=valgrind_sender_testError.log"
    valgrind_receiver="valgrind --leak-check=full --log-file=valgrind_receiver_testError.log"
    valgrind_link_sim="valgrind --leak-check=full --log-file=valgrind_link_testError.log"
fi

# On lance le receiver et capture sa sortie standard
$valgrind ./receiver :: 64341 2> logFile/receiver_testError.log > received_file &
receiver_pid=$!
...
$valgrind ./tests/link_sim -p 64342 -P 64341 -R -e 20 2>logFile/link_sim_testError.log &
link_sim_pid=$!
# On démarre le transfert
if ! $valgrind ./sender ::1 64342 < input_file 2> logFile/sender_testError.log ; then
  echo "Crash du sender!"
  cat sender_testError.log
