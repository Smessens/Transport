...
int initBufWindow(struct bufWindow *bufWindow){
    //bufWindow->tableau = malloc(sizeof( char *)*32);
    bufWindow->sizeMax = 32;
    bufWindow->size = 0;
    bufWindow->next = 0;
...
int main(int argc, char **argv) {
    ...
    while((loop == 1 || isEmpty(bufWindow1)>0)&& (timeoutCount < 5)){
        ...
        if( (timeoutCount>0) || (flagSendData>0)){// Timeout case
            sendData(sfd, numberWindow, bufWindow, len);
            if(timeoutCount>0){
                stat_packet_retransmitted+= numberWindow;
            }
            flagSendData = 0;
        }
