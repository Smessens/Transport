...
$valgrind ./receiver :: 12345 -s stats.csv 2> receiver.log 1> receiver.txt &
receiver_pid=$!
...
if ! $valgrind ./sender ::1 12345 -f tests/dataBigMessageTest.dat 2> sender.log ; then
  echo "Crash du sender!"
  cat sender.log
  err=1  # On enregistre l'erreur
fi

sleep 5 # On attend 5 secondes que le receiver finisse
...
if [[ "$(md5sum tests/dataBigMessageTest.dat | awk '{print $1}')" != "$(md5sum receiver.txt | awk '{print $1}')" ]]; then
